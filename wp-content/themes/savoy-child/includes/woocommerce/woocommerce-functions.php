...
function nm_shop_category_count( $string, $category ) {
    ...
    $html = '<mark class="count">' . sprintf( esc_html__( '%s products', 'nm-framework' ), $category->count ) . '</mark>';
    $html .= '<span class="lowest_price"> '.apply_filters('custom_lowest_pricing_category', $category->term_id ).' </span>';
    return $html;
